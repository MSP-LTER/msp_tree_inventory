# This script builds the MSP LTER tree inventory dataset
# author: Adrienne Keller
# dependencies: none
################################################################################

### Notes:
# a small subset of the data are excluded from the publicly available dataset,
#  so figures and statistics will vary slightly from published results which are
#  based on full dataset

# species names: I have manually edited names (via recode csv file, not included in R package) and have
...
CoCTU <- st_read("R/MSP_CountyCTUs/CountiesAndCTUs.shp")
...
source(file = "R/10_calc-species-diversity_EcoApps.R")
...
# read in SVI data
SVI <- st_read("R/SVI_2020_MSP/SVI_2020_MSP.shp")
SVI_df <- SVI %>% st_drop_geometry()
